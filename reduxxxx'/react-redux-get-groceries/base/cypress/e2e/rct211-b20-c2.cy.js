...
data.forEach(({ submission_link: url, id }) => {
  describe("Evaluation RCT-211-B120-E2", function () {
    ...
    it(`Check if grocery data is updated in the store`, () => {
      ...
      cy.intercept("GET", `https://dbioz2ek0e.execute-api.ap-south-1.amazonaws.com/mockapi/get-groceries`).as("getGroceries");

      cy.window().then(getCurrentState).its("groceryData").should("have.length", 0)
      ...
      cy.window().then(getCurrentState).its("groceryData").should("have.length", 10)
